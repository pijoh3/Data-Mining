...
fit2 = boosting(y~job+marital+education+balance+housing+
                  loan+contact+month+duration+campaign+poutcome, 
                data=bank, boos=T, mfinal=5, control=my.control)
...
pred = predict.boosting(fit2, newdata=bank)
cutoff = 0.11
...
pred2 = predict.boosting(fit2, newdata=bank)$prob
...
fit2 = boosting(y~job+marital+education+balance+housing+
                  loan+contact+month+duration+campaign+poutcome, 
                data=bank.train, boos=T, mfinal=100, control=my.control)
fit2$trees
...
#boosting방법의 분류기의 수와 오분류율#
evol.bank2 = errorevol(fit2, newdata = bank)
plot.errorevol(evol.bank2)
...
pred2 = predict.boosting(fit2, newdata=bank.train)$prob
...
pred2 = predict.boosting(fit2, newdata=bank.test)$prob
...
for(i in 1:V) {
  ...
  pred = predict.boosting(fit2, newdata=bank.train)
  yhat = ifelse(pred$prob[,2] > cutoff, 1, 0)
